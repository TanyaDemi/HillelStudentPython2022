# HW24. Матриця 2
"""
Необходимо создать матрицу (двухмерный список) М х N. M и N задаёт пользователь с клавиатуры.
Обязательные условия: 1. матрица создаётся при помощи list comprehension. При создании, список заполняется
случайными числами (используйте генератор случайных чисел)
2. необходимо посчитать сумму значений каждой строки (функцию sum() использовать НЕЛЬЗЯ).
Сумма выводится напротив соответствующих строк
3. необходимо посчитать сумму значений каждой колонки (функцию sum() использовать НЕЛЬЗЯ).
Сумма выводится под соответствующей колонкой
...
6. Вывод программы ДОЛЖЕН соответствовать примеру ниже (для форматирования вывода, Вам понадобится функция format())
7. задача считается выполненной ТОЛЬКО при строгом выполнении всех условий.
Введите кол-во строк: 15                        Введите кол-во колонок: 10
